...
// Client numbers are generated by the backend in format: initials-mmddyy[4-digit sequential]
// Example: kh1228250001 (kh = initials, 122825 = Dec 28 2025, 0001 = sequential)
const buildClientNumber = async (first: string, last: string, date?: string): Promise<string> => {
  try {
    const number = await invoke<string>("get_next_client_number", {
      first_name: first,
      last_name: last,
      date_str: date || null,
    });
    ...
    console.error("Error getting client number:", error);
    return "";
  }
};
                      ...
                      onSubmit={async (e) => {
                        ...
                        try {
                          ...
                          const generatedNumber = editingClientId 
                            ...
                            : await buildClientNumber(
                                clientForm.first_name.trim(),
                                clientForm.last_name.trim(),
                                onboardingDate
                              );
                          if (!editingClientId && !generatedNumber) {
                            setClientError("Client number could not be generated. Please try again.");
                            setBusy(false);
                            return;
                          }
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setWorkOrderNewClient((prev) => ({
                                  ...
                                  client_number: "", // Will be generated when client is created
                                }))
                              }
                            />
                          </label>
