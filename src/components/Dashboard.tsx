        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem", paddingBottom: "2rem" }}>
            ...
            <div className="hero">
                ...
                <div style={{ display: "flex", gap: "1rem", marginTop: "1rem" }}>
                    <div className="card" style={{ flex: 1, backgroundColor: "#fff9f0", borderColor: "#fce9cc" }}>
                        <div className="badge" style={{ background: "#e67f1e", color: "white" }}>Split Wood</div>
                        <div style={{ fontSize: "2rem", fontWeight: "bold", marginTop: "0.5rem" }}>
                            {woodSummary.split.toFixed(1)} <span style={{ fontSize: "1rem", fontWeight: "normal", color: "#666" }}>cords</span>
                        </div>
                    ...
                    <div className="card" style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: "2rem", fontWeight: "bold", marginTop: "0.5rem" }}>
                            {woodSummary.unsplit.toFixed(1)} <span style={{ fontSize: "1rem", fontWeight: "normal", color: "#666" }}>cords</span>
                        </div>
                    ...
                    {(session.isDriver || session.role === "volunteer") && (
                        <div className="card" style={{ flex: 1, border: "1px dashed #ccc" }}>
                            ...
                            <div className="stack" style={{ marginTop: "0.5rem", gap: "0.25rem" }}>
                                <div><strong>{userDeliveryHours.deliveries}</strong> deliveries</div>
                                <div><strong>{userDeliveryHours.hours.toFixed(1)}</strong> hours</div>
                            </div>
                        </div>
                    )}
                </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "1.5rem" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
                    ...
                    <div className="card">
                        ...
                        <div style={{
                            ...
                            {WEEKDAYS.map(d => <div key={d} style={{ fontWeight: "bold", fontSize: "0.85rem", color: "#666", paddingBottom: "0.5rem" }}>{d}</div>)}
                            ...
                            {upcomingDeliveries.map(d => (
                                <li key={d.id} style={{ display: "flex", justifyContent: "space-between" }}>
                                    <span>
                                        ...
                                        <span className="muted" style={{ marginLeft: "0.5rem" }}>{d.event_type}</span>
                                    </span>
                                    <span>{new Date(d.start_date).toLocaleDateString()}</span>
                                </li>
                            ))}
                            {upcomingDeliveries.length === 0 && <li className="muted">No upcoming events scheduled.</li>}
                        </ul>
                    </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
                    ...
                    <div className="card">
                        ...
                        <div className="stack" style={{ maxHeight: "250px", overflowY: "auto", gap: "0.75rem", marginTop: "0.5rem" }}>
                    ...
                    {isAdminOrLead && (
                        <>
                            <div className="card" style={{ borderColor: lowInventory.length > 0 ? "#e53935" : "#eee" }}>
                                <h3 style={{ color: lowInventory.length > 0 ? "#e53935" : "inherit" }}>
                                    Low Inventory
                                    {lowInventory.length > 0 && <span className="badge" style={{ backgroundColor: "#e53935", color: "white", marginLeft: "0.5rem" }}>{lowInventory.length}</span>}
                                </h3>
                                {lowInventory.length === 0 ? (
                                    <div className="muted" style={{ marginTop: "0.5rem" }}>All stock levels good.</div>
                                ) : (
                            ...
                            <div className="card">
                                ...
                                <ul className="list" style={{ marginTop: "0.5rem" }}>
                                    {availableStaff.length === 0 && <li className="muted">No staff scheduled for today.</li>}
