        ...
        if (this.state.hasError) {
            return (
                <div style={{ padding: 40, fontFamily: "sans-serif" }}>
                    ...
                    <div style={{ background: "#f9f4eb", padding: 20, borderRadius: 8, border: "1px solid #d7c6b6", overflow: "auto" }}>
                        <h3 style={{ marginTop: 0 }}>{this.state.error?.toString()}</h3>
                        <pre style={{ margin: 0 }}>{this.state.errorInfo?.componentStack}</pre>
                    </div>
                    <button
                        onClick={() => window.location.reload()}
                        style={{ marginTop: 20, padding: "10px 20px", background: "#2d6b3d", color: "white", border: "none", borderRadius: 8, cursor: "pointer", fontWeight: "bold" }}
                    >
                        Reload Application
                    </button>
                </div>
