...
export default function AdminPanel({ session }: AdminPanelProps) {
    ...
    return (
        <div className="stack">
            <div style={{ display: 'flex', gap: '1rem', borderBottom: '1px solid #ddd', paddingBottom: '0.5rem' }}>
                <button
            ...
            {error && <div className="pill" style={{ background: '#fbe2e2', color: '#b3261e' }}>{error} <button onClick={() => setError(null)}>x</button></div>}
                ...
                <div className="grid-2">
                    <div className="card">
                        ...
                        <form onSubmit={handleCreateMotd} className="stack">
                            ...
                            <button className="ping" type="submit" disabled={busy}>Publish</button>
                        </form>
                    </div>
                    <div className="card">
                        ...
                        <div className="stack" style={{ maxHeight: '400px', overflowY: 'auto' }}>
                            {motds.map(m => (
                                <div key={m.id} className="card-item" style={{ border: '1px solid #eee', padding: '0.5rem' }}>
                                    <div>{m.message}</div>
                                    ...
                                    <button className="ghost" style={{ color: '#b3261e', fontSize: '0.8rem' }} onClick={() => handleDeleteMotd(m.id)}>Delete</button>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="table">
                        ...
                        {changeRequests.map(r => (
                            <div key={r.id} className="table-row">
                                ...
                                <div>
                                    <button className="ghost" onClick={() => setSelectedRequest(r)}>Resolve/Reject</button>
                                </div>
                            </div>
                        ))}
                ...
                <div className="modal-overlay">
                    <div className="modal">
                        ...
                        <div className="actions">
                            <button className="ping" onClick={() => handleResolveRequest('resolved')} disabled={busy}>Resolve (Done)</button>
                            <button className="ghost" style={{ color: '#b3261e' }} onClick={() => handleResolveRequest('rejected')} disabled={busy}>Reject</button>
                            <button className="ghost" onClick={() => setSelectedRequest(null)}>Cancel</button>
                        </div>
                    </div>
                </div>
            )}
